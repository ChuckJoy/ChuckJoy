var DiffCamEngine = (function() {
	...
	function init(options) {
		...
		requestWebcam('environment');
	}

	function requestWebcam(cam) {  // 'environment'  or 'user'
		var constraints = {
			audio: false,
	...
	function capture() {
		...
		captureContext.ReadFrequently=true;  //  attribute set to true. See: https://html.spec.whatwg.org/multipage/canvas.html#concept-canvas-will-read-frequently
		var captureImageData = captureContext.getImageData(0, 0, captureWidth, captureHeight);  

		// diff current capture over previous capture, leftover from last time
		diffContext.globalCompositeOperation = 'difference';
		diffContext.drawImage(video, 0, 0, diffWidth, diffHeight);
		diffContext.ReadFrequently=true;  //  attribute set to true. See: https://html.spec.whatwg.org/multipage/canvas.html#concept-canvas-will-read-frequently
